...
# Maximum number of missing files before prompting the user
max_missing_files_prompt = 100
...
try:
    ...
    # Copy missing files to the specified folder if there are not too many
    if missing_count <= max_missing_files_prompt:
        for file in missing_files:
            source_path = source_files[file][1]
            destination_path = os.path.join(missing_files_path, file)
            shutil.copy2(source_path, destination_path)
        logging.info(f"Missing files copied to {missing_files_path}")
    else:
        logging.warning(f"Excessive number of missing files ({missing_count}). Files were not copied.")
